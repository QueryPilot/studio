...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Testing PostgreSQL special types with text casting...");
    
    // Create connection pool
    let pool = PgPool::connect(connection_string).await?;
    let adapter = PostgresAdapter::new(pool);
    
    // Test query with special types - cast to text for proper handling
    let query = r#"
        SELECT 
            ARRAY[1, 2, 3]::INT4[] as int_array,
            'hello world'::tsvector::text as search_vector,
            tstzrange('2024-01-01', '2024-12-31')::text as date_range,
            'key1=>val1, key2=>val2'::hstore::text as metadata
        LIMIT 1
    "#;
    ...
    println!("Executing query with ::text casting...");
    ...
    println!("\nNow testing without ::text casting to see raw format...");
    
    // Query without casting to see what we get
    let query2 = r#"
        SELECT 
    ...
    match adapter.begin_query(query2, None, opts2).await {
        Ok(cursor) => {
            println!("Query successful!");
            for (i, row) in cursor.rows.iter().enumerate() {
                println!("\nRow {}:", i + 1);
                for (j, cell) in row.iter().enumerate() {
                    ...
                    let value_str = match &cell.value {
                        Some(val) => {
                            // Show the raw value in debug format
                            format!("{:?}", val)
                        },
                        None => "NULL".to_string(),
                    };
    ...
    println!("\nSpecial types decode test completed!");
    Ok(())
}
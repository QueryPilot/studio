...
pub fn generate_insert_template(table: &TableInfo, columns: &[ColumnInfo]) -> String {
    ...
    let placeholders: Vec<_> = col_names
        .iter()
        .enumerate()
        .map(|(i, _)| format!("${}", i + 1))
        .collect();
...
pub fn generate_insert_for_columns(table: &TableInfo, columns: &[&str]) -> String {
    let schema_prefix = table
        .schema
        .as_ref()
        .map(|s| format!("{}.", s))
    ...
    let placeholders: Vec<_> = columns
        .iter()
        .enumerate()
        .map(|(i, _)| format!("${}", i + 1))
        .collect();
...
pub fn generate_update_template(table: &TableInfo, columns: &[ColumnInfo]) -> String {
    ...
    let pk_cols: Vec<_> = columns.iter().filter(|c| c.is_primary_key).collect();

    let where_clause = if pk_cols.is_empty() {
        "WHERE ".to_string()
    } else {
        let conditions: Vec<_> = pk_cols
            .iter()
            .enumerate()

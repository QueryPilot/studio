    ...
    let params: Vec<String> = func
        ...
        .map(|(i, p)| {
            format!(
                "${{{}: {}}}",
                i + 1,
                p.name.as_deref().unwrap_or(&p.data_type)
            )
        })
        .collect();
...
fn get_postgres_builtins() -> Vec<BuiltinFunctionHelp> {
    vec![
        ...
        BuiltinFunctionHelp {
            ...
            examples: vec!["SELECT NULLIF(col, '')".to_string()],
        },
        BuiltinFunctionHelp {
            ...
            examples: vec!["SELECT ARRAY_AGG(name ORDER BY name)".to_string()],
        },
        BuiltinFunctionHelp {
            ...
            examples: vec!["SELECT STRING_AGG(name, ', ')".to_string()],
        },
        BuiltinFunctionHelp {
            name: "ROW_NUMBER".to_string(),
            signature: "ROW_NUMBER() OVER (...)".to_string(),
            description: "Assigns sequential row numbers".to_string(),
            examples: vec!["SELECT ROW_NUMBER() OVER (ORDER BY id)".to_string()],
        },
    ]
}
...
fn get_mysql_builtins() -> Vec<BuiltinFunctionHelp> {
    vec![
        BuiltinFunctionHelp {
            ...
            examples: vec!["SELECT IFNULL(col, 'default')".to_string()],
        },
        BuiltinFunctionHelp {
            ...
            examples: vec!["SELECT GROUP_CONCAT(name SEPARATOR ', ')".to_string()],
        },
    ]
}

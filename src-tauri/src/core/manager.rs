...
impl ConnectionManager {
    ...
    pub fn get_connection(&self, conn_id: &str) -> Option<impl std::ops::Deref<Target = LiveConnection> + '_> {
    ...
    pub async fn disconnect_all(&self) -> Result<()> {
        let keys: Vec<String> = self.connections.iter()
    ...
    fn create_adapter(&self, profile: &ConnectionProfile) -> Result<Box<dyn crate::core::adapter::DbAdapter>> {
    ...
    pub fn get_query(&self, query_id: &str) -> Option<impl std::ops::Deref<Target = QueryHandle> + '_> {
    ...
    pub fn get_connection_stats(&self, conn_id: &str) -> Option<ConnectionStats> {
        self.connections.get(conn_id).map(|conn| ConnectionStats {
            ...
            last_used: conn.last_used.try_read().map(|t| *t).unwrap_or(conn.created_at),
        })
    }

...
impl DuckDbAdapter {
    ...
    async fn execute_sql_backup(
        ...
        let combine_result = (|| -> Result<(), AppError> {
        ...
        // Propagate any error from the combine step
        combine_result?;
    ...
    async fn execute_binary_restore(
        ...
        // Step 1: Create safety backup of current DB
        let _ = progress
            .send(BackupProgress::Progress {
                current: 1,
                total: 4,
                message: "Creating safety backup of current database...".to_string(),
            })
            .await;

        let safety_backup = db_path.with_extension("duckdb.restore-backup");
        ...
        // Step 3: Copy backup file over the database
        let _ = progress
            .send(BackupProgress::Progress {
                current: 3,
                total: 4,
        ...
        // Step 4: Try to reconnect — if it fails, restore the safety backup
        let _ = progress
            .send(BackupProgress::Progress {
                current: 4,
                total: 4,
        ...
        if let Err(e) = self.reconnect_internal().await {
            // Restore original database from safety backup
            let _ = tokio::fs::copy(&safety_backup, &db_path).await;
            let _ = self.reconnect_internal().await;
            let _ = tokio::fs::remove_file(&safety_backup).await;

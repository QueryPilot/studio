...
impl PostgresTypeConverter {
    ...
    pub fn value_to_cell_with_type_hint(
        row: &Row,
        idx: usize,
        type_hint: Option<&CellValueType>,
    ) -> Result<CellValue> {
        ...
        let is_tsvector_type =
            matches!(cell_type, CellValueType::TsVector | CellValueType::TsQuery)
        ...
        let display_value = if is_tsvector_type && matches!(*pg_type, Type::TEXT | Type::VARCHAR) {
            ...
            match row.try_get::<_, Option<String>>(idx) {
                ...
                Err(e) => {
                    eprintln!(
                        "DEBUG: Error getting tsvector as text at column {}: {:?}",
                        idx, e
                    );
                    return Ok(CellValue::null());
                }
            }
        } else if matches!(cell_type, CellValueType::Money)
            && matches!(*pg_type, Type::TEXT | Type::VARCHAR)
        {
            ...
            cell_type,
            CellValueType::Range(_) | CellValueType::Multirange(_)
        ) && matches!(*pg_type, Type::TEXT | Type::VARCHAR)
        {
                ...
                Type::BOOL_ARRAY => {
                    if let Ok(Some(arr)) = row.try_get::<_, Option<Vec<bool>>>(idx) {
                        format_postgres_array(
                            &arr.iter().map(|v| v.to_string()).collect::<Vec<_>>(),
                        )
                    } else {
                        return Ok(CellValue::null());
                    }
                ...
                Type::INT4_RANGE => {
                    if let Ok(Some(val)) = row.try_get::<_, Option<String>>(idx) {
                        match PostgresTypeParser::parse_range(&val, &CellValueType::Integer) {
                            Ok(parsed) => {
                                return Ok(CellValue {
                                    value_type: CellValueType::Range(Box::new(
                ...
                _ => {
                    ...
                    match type_name {
                        "hstore" => {
                            ...
                            } else if let Ok(Some(map)) =
                                row.try_get::<_, Option<HashMap<String, Option<String>>>>(idx)
                            {
                        ...
                        _ if !is_standard_postgres_type(type_name) => {
                            ...
                            match row.try_get::<_, Option<String>>(idx) {
                                ...
                                Err(e) => {
                                    ...
                                    return Ok(CellValue {
                                        value_type: CellValueType::CustomType(
                                            type_name.to_string(),
                                        ),
                                        raw_value: None,

...
async fn test_query_execution() {
    ...
    let handle = adapter.open_query("SELECT COUNT(*) FROM users").await.unwrap();
...
async fn test_postgresql_basic_types() {
    ...
    let handle = adapter.open_query("SELECT id, title, status FROM todos LIMIT 1").await.unwrap();
    let chunk = adapter.fetch_page(&handle, 1).await.unwrap();
...
async fn test_postgresql_json_types() {
    ...
    let handle = adapter.open_query("SELECT tags, custom_fields FROM todos WHERE tags IS NOT NULL LIMIT 1").await.unwrap();
    let chunk = adapter.fetch_page(&handle, 1).await.unwrap();
...
async fn test_postgresql_advanced_types() {
    ...
    let handle = adapter.open_query("SELECT collaborator_ids FROM todos WHERE collaborator_ids IS NOT NULL LIMIT 1").await.unwrap();
    let chunk = adapter.fetch_page(&handle, 1).await.unwrap();
...
async fn test_null_handling() {
    ...
    adapter.execute("CREATE TEMP TABLE null_test (id INTEGER, value TEXT)").await.unwrap();
    adapter.execute("INSERT INTO null_test VALUES (1, NULL), (2, 'not null')").await.unwrap();
    
    let handle = adapter.open_query("SELECT * FROM null_test ORDER BY id").await.unwrap();
...
async fn test_introspection_functions() {
    ...
    assert!(constraints.iter().any(|c| matches!(c.constraint_type, ConstraintType::PrimaryKey)));
...
async fn test_large_result_pagination() {
    ...
    let handle = adapter.open_query("SELECT id, title, description FROM todos LIMIT 100").await.unwrap();
    ...
    println!("Fetched {} total rows in {} iterations", total_rows, iterations);

...
def create_schema(conn: sqlite3.Connection) -> None:
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_activity_logs_todo_id ON activity_logs(todo_id)"
    )
...
def seed_data(conn: sqlite3.Connection, user_count: int = 100) -> None:
    ...
    for i in range(1, user_count + 1):
        user_data = (
            ...
            f"Bio for user {i}. "
            + "Lorem ipsum dolor sit amet. " * random.randint(1, 3),
            1 if random.random() > 0.1 else 0,
            1 if random.random() > 0.3 else 0,
            f"+1555{random.randint(0, 9999999):07d}",
            (
                datetime.date.today()
                - datetime.timedelta(days=random.randint(365 * 18, 365 * 70))
            ).isoformat(),
            json.dumps(
                {
    ...
    for user_id in users:
        ...
        for j in range(1, todo_count + 1):
            ...
            due_date = (
                datetime.date.today()
                + datetime.timedelta(days=random.randint(-180, 365))
            ).isoformat()
            ...
            tags = json.dumps(
                [
                    random.choice(["work", "personal", "urgent", "learning"]),
            ...
            custom_fields = json.dumps(
                {
                    ...
                    "project_code": f"PRJ-{random.randint(0, 9999):04d}"
            ...
            todo_data = (
                ...
                * random.randint(1, 5),
                ...
                100
                if status == "completed"
                else (random.randint(0, 99) if status == "in_progress" else 0),
                tags,
                attachments,
                checklist,
                custom_fields,
                ...
                random.choice(["daily", "weekly", "monthly", "yearly"])
                ...
                "Lorem ipsum dolor sit amet. " * random.randint(5, 15)
                if random.random() > 0.5
                else None,
                f"Note: Important information. " * random.randint(1, 3)
                if random.random() > 0.7
                else None,
    ...
    for _ in range(min(500, len(todo_ids))):
        ...
        """,
            (
                todo_id,
                user_id,
                f"Comment: {random.choice(comment_texts)}",
                1 if random.random() > 0.8 else 0,
            ),
        )

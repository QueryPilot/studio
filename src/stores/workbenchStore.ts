...
import { GridNode, PanelContent, SplitAction, Direction, DragDropContext } from '@/types/workbench';
import {
  splitPanel,
  closePanel,
  resizePanel,
...
interface WorkbenchStore {
  ...
  addTab: (panelId: string, tabId: string, tabData?: any) => void;
  ...
  persist(
    (set, get) => ({
      ...
      focusAdjacentPanel: (direction) => {
        ...
        const adjacentId = getAdjacentPanel(layoutTree, focusedPanelId, direction);
        if (adjacentId) {
      ...
      saveLayout: () => {
        const { layoutTree } = get();
        if (layoutTree) {
          localStorage.setItem('workbench-layout-backup', JSON.stringify(layoutTree));
        }
      },
      ...
      removeTab: (panelId, tabId) => {
        ...
        const newActiveTab = panel.activeTabId === tabId 
          ? newTabIds[0] || '' 
          : panel.activeTabId;
...
function updatePanelContents(tree: GridNode, contents: Map<string, PanelContent>): GridNode {
  ...
  if (tree.type === 'branch' && tree.children) {
    return {
      ...tree,
      children: tree.children.map(child => updatePanelContents(child, contents))
    };
  }

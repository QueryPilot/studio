  ...
  persist(
    immer((set, get) => ({
      ...
      removeConnectionFromWorkspace: (workspaceId, connectionId) => {
        set((state) => {
          const workspace = state.workspaces.get(workspaceId);
          if (workspace) {
            workspace.connectionIds = workspace.connectionIds.filter(id => id !== connectionId);
            workspace.updatedAt = new Date();
            ...
            if (workspace.activeTabId && tabsToClose.includes(workspace.activeTabId)) {
        ...
        set((state) => {
          ...
          if (workspace.tabs.size >= workspace.settings.maxTabsOpen) {
            ...
            if (oldestTabId) {
              ...
              workspace.tabOrder = workspace.tabOrder.filter(id => id !== oldestTabId);
            }
          }
      ...
      closeTabsToRight: (workspaceId, tabId) => {
        set((state) => {
          ...
          if (workspace.activeTabId && tabsToRemove.includes(workspace.activeTabId)) {
            workspace.activeTabId = tabId;
          }
    ...
    })),
    {
      ...
      partialize: (state): SerializableWorkspaceStore => ({
        workspaces: Array.from(state.workspaces.entries()).map(([id, ws]): [string, SerializableWorkspaceState] => [
          id,
          {
            ...ws,
            tabs: Array.from(ws.tabs.entries()).map(([tabId, tab]): [string, SerializableTabState] => [
              tabId,
              {
                ...tab,
                ui: {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        state.workspaces.forEach(([id, ws]: [string, any]) => {

...
const IconPicker = ({ value, onChange, className }: IconPickerProps) => {
  const [activeTab, setActiveTab] = React.useState<"emoji" | "icons" | "upload">("emoji");
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      />
      <PopoverContent className="w-[350px] p-0 overflow-hidden border-border/40 shadow-xl" align="start">
        ...
        {activeTab === "emoji" ? (
          <Command className="w-full border-none rounded-none">
            <div className="flex items-center px-3 py-2 border-b gap-2">
              ...
              <div className="flex items-center gap-1 border-l pl-2 ml-1">
                ...
                <div 
                  className="w-8 h-8 flex items-center justify-center text-2xl leading-none transition-all duration-200"
                >
                  {hoveredEmoji || value}
                </div>
              </div>
              ...
              {Object.entries(EMOJI_CATEGORIES).map(([category, emojis]) => {
                 ...
                 return (
                   <CommandGroup 
                     ...
                     className="mb-2"
                   >
                     <div ref={(el) => { categoryRefs.current[category] = el; }} className="w-0 h-0" />
            ...
            <div className="flex items-center gap-1 p-2 border-t overflow-x-auto no-scrollbar scrollbar-none bg-muted/20">
              {Object.entries(CATEGORY_ICONS).map(([category, icon]) => (
                <button
                  ...
                  className="flex items-center justify-center w-7 h-7 rounded hover:bg-accent text-sm grayscale opacity-70 hover:grayscale-0 hover:opacity-100 transition-all flex-shrink-0"
                  title={category}
                >
                  {icon}
                </button>
          ...
          <div className="h-[320px] flex flex-col items-center justify-center text-muted-foreground gap-2 p-4 text-center">
            {activeTab === "icons" ? <IconIcons className="w-8 h-8 opacity-20" /> : <IconUpload className="w-8 h-8 opacity-20" />}
            <p className="text-xs">
              {activeTab === "icons" ? "Icon library coming soon" : "Custom uploads coming soon"}
            </p>
          </div>
        )}
      </PopoverContent>

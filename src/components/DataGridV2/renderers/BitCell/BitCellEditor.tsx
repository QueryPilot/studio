...
export const BitCellEditor: React.FC<BitCellEditorProps> = ({
  ...
  const isFixedLength = value.data.kind === "bit-cell" && expectedLength !== undefined;
  ...
  const isValidInput = useMemo(() => {
    ...
    const validLength = !isFixedLength || isValidBitLength(binaryValue, expectedLength);
    ...
    <div className="w-full flex flex-col relative click-outside-ignore z-50 min-w-[320px]">
      ...
      <div className="px-2 py-1.5 flex items-center gap-2">
        <div className="flex-1 text-[10px] text-muted-foreground">
          {totalBits > 0 && (
            <>
              {totalBits} bits, {setBits} set ({Math.round((setBits / totalBits) * 100)}%)
            </>
          )}
      ...
      {!isValidInput && (
        <div className="px-2 pb-1 text-[10px] text-destructive leading-tight">
          {!isValidBinary(binaryValue) && "Invalid binary string (only 0 and 1 allowed). "}
          {isFixedLength &&

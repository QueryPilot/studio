...
export const GeometryCellEditor: React.FC<GeometryCellEditorProps> = ({
  ...
  const validation = useMemo(() => {
    ...
    if (!geometryType) {
      return { isValid: true, type: null, srid: null, coordinates: "", error: undefined };
    }
  ...
  const commitCurrentValue = useCallback(() => {
    ...
    commit(currentValue);
  }, [commit, getCurrentValue, initialValue, validation.isValid, onFinishedEditing]);
          ...
          <div className="px-2 py-1.5 border-b border-border/30 flex items-center gap-2">
            ...
            <div className="w-32">
              ...
              <Select
                ...
                onValueChange={handleSridChange}
              >
                <SelectTrigger className="h-6 text-[10px]">
                  <SelectValue placeholder="No SRID" />
                </SelectTrigger>
                <SelectContent>

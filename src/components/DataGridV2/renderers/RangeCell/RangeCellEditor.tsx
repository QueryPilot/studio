...
export const RangeCellEditor: React.FC<RangeCellEditorProps> = ({
  ...
  const initialValue = useMemo(() => value.data.value ?? "", [value.data.value]);
  ...
  const placeholders = useMemo(() => getPlaceholder(elementType), [elementType]);
  ...
  const rangeValid = useMemo(() => {
    ...
    const bounds: Bounds = `${lowerInclusive ? "[" : "("}${upperInclusive ? "]" : ")"}`;
    return isValidRange(lowerVal, upperVal, bounds, elementType);
  }, [lower, upper, lowerEmpty, upperEmpty, lowerInclusive, upperInclusive, elementType]);
  ...
  const commitCurrentValue = useCallback(() => {
    ...
    commit(currentValue);
  }, [commit, getCurrentValue, initialValue, isValid, lowerEmpty, upperEmpty, nullable, onFinishedEditing]);
      ...
      <div className="px-2 py-1.5 border-b border-border/30">
        <div className="flex items-center gap-2 mb-1">
          ...
          <Select
            ...
            onValueChange={(v) => setLowerInclusive(v === "inclusive")}
          >
          ...
          <div className="flex items-center gap-1 ml-auto">
            <Checkbox
              id="lower-empty"
              checked={lowerEmpty}
              onCheckedChange={(checked) => setLowerEmpty(checked === true)}
              className="h-3 w-3"
            />
            <Label htmlFor="lower-empty" className="text-[10px] text-muted-foreground">
              Unbounded
            </Label>
          </div>
        </div>
      ...
      <div className="px-2 py-1.5 border-b border-border/30">
        <div className="flex items-center gap-2 mb-1">
          ...
          <Select
            ...
            onValueChange={(v) => setUpperInclusive(v === "inclusive")}
          >
            <SelectTrigger className="h-5 w-20 text-[10px]">
              <SelectValue />
            </SelectTrigger>
          ...
          <div className="flex items-center gap-1 ml-auto">
            <Checkbox
              id="upper-empty"
              checked={upperEmpty}
              onCheckedChange={(checked) => setUpperEmpty(checked === true)}
              className="h-3 w-3"
            />
            <Label htmlFor="upper-empty" className="text-[10px] text-muted-foreground">
              Unbounded
            </Label>
          </div>
        </div>

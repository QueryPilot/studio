...
export default function AIPreferencesPanel() {
  ...
  const setIncludeSchemaContext = useByokStore(
    (s) => s.setIncludeSchemaContext,
  );
  ...
  const setPreferredPackageManager = useAcpStore(
    (s) => s.setPreferredPackageManager,
  );
  ...
  const hasNpmPackages = useMemo(
    () =>
      availableAgents.some((a) =>
        a.packages.some((p) => p.managerType === "npm"),
      ),
    [availableAgents],
  );
  ...
  const handleInstallPackage = useCallback(
    async (pkg: PackageInfo, agentId: string) => {
      ...
      try {
        const pm =
          pkg.managerType === "brew" ? "brew" : preferredPackageManager;
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-8 pb-8 pt-6 space-y-6">
            ...
            {hasNpmPackages && (
              <div className="flex items-center gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    setPreferredPackageManager(v as NpmPackageManager);
                  }}
                >
              ...
              {availableAgents.map((agent) => {
                ...
                return (
                  <div key={agent.id}>
                    ...
                    <div
                      onClick={() => {
                        if (agent.installed) selectAgent(agent.id);
                      }}
                      className={cn(
                        ...
                        {!agent.installed ? (
                          <Button
                            ...
                            disabled={packages.some(
                              (p) =>
                                packageStates[p.name]?.status === "installing",
                            )}
                          >
                            {packages.some(
                              (p) =>
                    ...
                    {packages.length > 0 && (
                      <div className="pb-2">
                        {packages.map((pkg) => {
                          const state = packageStates[pkg.name] ?? {
                            status: "idle" as InstallStatus,
                          };
                          const isInstalled =
                            pkg.installed || state.status === "success";
                          const needsUpdate =
                            pkg.updateAvailable && state.status !== "success";
                            ...
                            >
                              ...
                              {isDone ? (
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    void handleInstallPackage(pkg, agent.id)
                                  }
                                ...
                                >
                                  {state.status === "installing" ? (
                                    ...
                                    <>
                                      ...
                                      <span className="ml-0.5">Retry</span>
                                    </>
                                  ) : needsUpdate ? (
                                    <>
                                      <IconArrowUp className="h-3 w-3 text-amber-500" />
                                      <span className="ml-0.5">Update</span>
                                    </>
                                  ) : (
                                    <>
                                      <IconDownload className="h-3 w-3" />
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canFetchModels && (
                    <Button
                      ...
                      disabled={
                        isFetchingModels || (config.requiresApiKey && !apiKey)
                      }
                    >
                      {isFetchingModels ? (
                        <IconLoader2 className="h-3 w-3 animate-spin" />
                ...
                {displayModels.map((m) => (
                  <button
                    ...
                    className={cn(
                      ...
                      m.id === modelId ? "bg-primary/5" : "hover:bg-accent/50",
                    )}
                  >
                    <span
                      className={cn(
        ...
        <section className="space-y-3">
          ...
          <div className="flex items-center justify-between py-3 border rounded-xl px-4">
            ...
            <Switch
              checked={skipApprovalGate}
              onCheckedChange={setSkipApprovalGate}
            />
          </div>

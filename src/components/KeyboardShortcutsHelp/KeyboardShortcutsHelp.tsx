...
import { CommandDescriptor } from '@/types/command';
...
export function KeyboardShortcutsHelp(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const disposers = [
      commandService.onDidRegister(() => updateShortcuts()),
      commandService.onDidUnregister(() => updateShortcuts()),
      keybindingService.onDidRegister(() => setVersion((value) => value + 1)),
      keybindingService.onDidUnregister(() => setVersion((value) => value + 1)),
      keybindingService.onDidChange(() => setVersion((value) => value + 1)),
    ];
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return label.includes(lower) || category.includes(lower) || keybinding.includes(lower);
    });
  }, [query, rows]);
  ...
  const grouped = useMemo(() => {
    ...
    return Array.from(groups.entries()).map(([category, items]) => [
      category,
      items.sort((left, right) => left.command.label.localeCompare(right.command.label)),
    ]) as [string, ShortcutRow[]][];
  }, [filteredRows]);
    ...
    <Dialog open={keyboardShortcutsOpen} onOpenChange={setKeyboardShortcutsOpen}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>Quick reference for commands and keybindings.</DialogDescription>
        </DialogHeader>
                    ...
                    {items.map((row) => (
                      ...
                      >
                        <div className="flex flex-col">
                          <span className="text-sm font-medium">{row.command.label}</span>
...
function pickBestKeybinding(
  ...
  return matches.sort((left, right) => (right.weight ?? 0) - (left.weight ?? 0))[0];
}

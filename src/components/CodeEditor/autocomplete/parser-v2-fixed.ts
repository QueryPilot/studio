...
export class SqlQueryParser {
  ...
  private findQueryBoundaries(text: string, pos: number): { start: number; end: number } {
  ...
  private findSemicolonsNotInStrings(line: string | undefined): number[] {
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      else if (char === '`' && prevChar !== '\\') inBacktick = !inBacktick;
  ...
  private identifyClauseFromAST(tree: any, pos: number, text: string): QueryContext['currentClause'] {
  ...
  private identifyClauseFromText(text: string, pos: number): QueryContext['currentClause'] {
    ...
    const fromTablePattern = /\bFROM\s+([a-zA-Z_][\w.]*|\[[^\]]+\]|`[^`]+`|"[^"]+")(?:\s+(?:AS\s+)?[a-zA-Z_]\w*)?\s*$/i;
    ...
    let bestMatch: { clause: QueryContext['currentClause']; position: number } | null = null;
  ...
  private extractTablesFromAST(
    ...
    if (tables.length === 0) {
      ...
      const fromPattern = /\bFROM\s+([a-zA-Z_][\w.]*|\[[^\]]+\]|`[^`]+`|"[^"]+")(?:\s+(?:AS\s+)?([a-zA-Z_]\w*))?/gi;
      const joinPattern = /\b(?:LEFT|RIGHT|INNER|OUTER|FULL)?\s*JOIN\s+([a-zA-Z_][\w.]*|\[[^\]]+\]|`[^`]+`|"[^"]+")(?:\s+(?:AS\s+)?([a-zA-Z_]\w*))?/gi;
  ...
  private isTableContext(node: SyntaxNode): boolean {
    ...
    while (parent) {
      ...
      if (name.includes('FROM') || name.includes('JOIN') || name.includes('INTO')) {
        return true;
      }
  ...
  private parseAliasesFromAST(
    ...
    tree.iterate({
      enter: (node: SyntaxNode) => {
        ...
        if (node.name === 'Alias' || node.name === 'TableAlias') {
          ...
          if (tableNode) {
            const tableText = state.doc.sliceString(tableNode.from, tableNode.to);
    ...
    const patterns = [
      ...
      /([^\s,]+)\s+([a-zA-Z_]\w*)(?=\s*(?:,|LEFT|RIGHT|INNER|WHERE|GROUP|ORDER|$))/gi
    ];
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(queryText)) !== null) {
        ...
        if (aliasPart && !this.SQL_KEYWORDS.has(aliasPart.toUpperCase())) {
  ...
  private parseCTEs(queryText: string): Map<string, TableReference> {
    ...
    const cteRegex = /WITH\s+(?:RECURSIVE\s+)?([a-zA-Z_]\w*)\s+(?:\([^)]*\))?\s+AS\s*\(/gi;
  ...
  private extractJoinConditions(
    ...
    const joinPattern = /JOIN\s+[^\s]+(?:\s+[a-zA-Z_]\w*)?\s+ON\s+([^WHERE|GROUP|ORDER|HAVING]+)/gi;
  ...
  private getCurrentContext(state: EditorState, pos: number): { database?: string; schema?: string } {
    ...
    const schemaMatch = /(?:SET|USE)\s+SCHEMA\s+([a-zA-Z_]\w*)/gi.exec(textBefore);
  ...
  private getPrefix(state: EditorState, pos: number): string {
    ...
    return state.doc.sliceString(word.from, pos);
  }
}
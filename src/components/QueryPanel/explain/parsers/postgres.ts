import type { ExplainNode, JitInfo, ParsedExplain } from "../types";
...
export function parsePostgresExplain(rows: unknown[][]): ParsedExplain {
  nodeIdCounter = 0;
  const lines: string[] = [];
  ...
  function convertNode(jsonNode: JsonPlanNode): ExplainNode {
    ...
    if (jsonNode["Full-sort Groups"]) {
      ...
      node.fullSortGroups = {
        ...
        memoryType: Array.isArray(fg["Sort Methods Used"])
          ? fg["Sort Methods Used"].join(", ")
          : "unknown",
      };
    }
    if (jsonNode["Pre-sorted Groups"]) {
      ...
      node.preSortedGroups = {
        ...
        memoryType: Array.isArray(pg["Sort Methods Used"])
          ? pg["Sort Methods Used"].join(", ")
          : "unknown",
      };
    }

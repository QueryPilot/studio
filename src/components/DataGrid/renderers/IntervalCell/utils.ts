...
export function parseInterval(input: string | null | undefined): IntervalParts {
  ...
  if (timeMatch) {
    ...
    parts.hours = parseInt(timeMatch[2] ?? "0", 10);
    parts.minutes = parseInt(timeMatch[3] ?? "0", 10);
    if (timeMatch[4]) {
...
function parseIso8601Duration(input: string): IntervalParts {
  ...
  if (datePart) {
    ...
    if (yearMatch?.[1]) parts.years = parseInt(yearMatch[1], 10);

    const monthMatch = datePart.match(/(\d+)M/);
    if (monthMatch?.[1]) parts.months = parseInt(monthMatch[1], 10);

    const dayMatch = datePart.match(/(\d+)D/);
    if (dayMatch?.[1]) parts.days = parseInt(dayMatch[1], 10);
  }

  // Parse time part
  if (timePart) {
    const hourMatch = timePart.match(/(\d+)H/);
    if (hourMatch?.[1]) parts.hours = parseInt(hourMatch[1], 10);

    const minMatch = timePart.match(/(\d+)M/);
    if (minMatch?.[1]) parts.minutes = parseInt(minMatch[1], 10);

    const secMatch = timePart.match(/([\d.]+)S/);
    if (secMatch?.[1]) parts.seconds = parseFloat(secMatch[1]);
  }

...
const DocumentCollectionDataGrid = memo(function DocumentCollectionDataGrid({
  ...
  // AI filter generator for MongoDB
  const generateAIFilter = useCallback(
    ...
    > => {
      try {
        ...
        const fullPrompt = `You are a MongoDB filter assistant. Generate a MongoDB query filter document.

User request: ${prompt}

Collection: ${collection}
Fields: ${filterColumns.map((c) => `${c.name} (${c.dataType})`).join(", ")}

RULES:
1. Respond with ONLY valid JSON in this format: {"description": "short explanation", "query": "the MongoDB filter"}
2. The "query" field must be a valid MongoDB filter document (JSON), e.g. {"status": "active", "price": {"$gt": 100}}
3. Use MongoDB query operators: $gt, $gte, $lt, $lte, $ne, $in, $nin, $regex, $exists, $and, $or, etc.
4. For sorting/limiting (top N), use $expr or aggregation-style filters where possible.
5. ONLY use field names that exist in the schema above.
6. No markdown, no code fences, no explanation outside the JSON.`;
        ...
        return {
          error: error instanceof Error ? error.message : "Failed to generate AI filter",
        };
      }
    },

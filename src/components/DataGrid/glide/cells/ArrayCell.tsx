import {
  GridCellKind,
  type CustomCell,
  type DrawArgs,
  type CustomRenderer,
  type Theme,
} from "@glideapps/glide-data-grid";
import { truncateTextToWidth } from "../types";

type ArrayCellData = {
  kind: "array-cell";
  value: unknown[] | string | null;
  metadata?: { element_type?: string } & Record<string, unknown>;
};

export type ArrayCustomCell = CustomCell<ArrayCellData>;

export const ArrayCell: CustomRenderer<ArrayCustomCell> = {
  kind: GridCellKind.Custom,
  isMatch: (cell: CustomCell): cell is ArrayCustomCell => {
    return (cell as any)?.data?.kind === "array-cell";
  },
  draw: (args: DrawArgs<ArrayCustomCell>, cell: ArrayCustomCell) => {
    const ctx = args.ctx as CanvasRenderingContext2D;
    const rect = args.rect;
    const theme = args.theme as Theme;
    const { value } = cell.data;

    let text = "NULL";
    if (value !== null && value !== undefined) {
      if (Array.isArray(value)) {
        try {
          text = JSON.stringify(value);
        } catch {
          text = `[${value.length} items]`;
        }
      } else if (typeof value === "string") {
        text = value;
      } else {
        text = String(value);
      }
    }

    const baseFont = theme.baseFontStyle || "12px sans-serif";
    const isNull = value == null;
    ctx.fillStyle = theme.textDark;
    ctx.font = isNull ? `italic ${baseFont}` : baseFont;
    if (isNull) ctx.globalAlpha = 0.55;
    ctx.textAlign = "left";
    ctx.textBaseline = "middle";
    const padding = theme.cellHorizontalPadding ?? 6;
    const maxTextWidth = Math.max(0, rect.width - padding * 2);
    const display = truncateTextToWidth(text, maxTextWidth, baseFont);
    ctx.save();
    ctx.beginPath();
    ctx.rect(rect.x, rect.y, rect.width, rect.height);
    ctx.clip();
    ctx.fillText(display, rect.x + padding, rect.y + rect.height / 2);
    ctx.restore();
    return true;
  },
  provideEditor: () => ({
    editor: (props) => {
      const v = props.value.data.value;
      const text = Array.isArray(v)
        ? JSON.stringify(v, null, 2)
        : String(v ?? "");
      return (
        <div className="p-2 text-xs min-w-[260px] max-w-[520px] max-h-[360px] overflow-auto">
          <pre className="whitespace-pre-wrap break-words">{text}</pre>
        </div>
      );
    },
    disablePadding: true,
  }),
};

...
export const GridRenderer: React.FC<GridRendererProps> = ({ 
  ...
  const [localSplitRatio, setLocalSplitRatio] = useState(node.splitRatio || 0.5);
  ...
  const handleResize = useCallback((mousePos: number) => {
    if (!containerRef.current || node.type !== 'branch') return;
    
    const containerRect = containerRef.current.getBoundingClientRect();
    const containerStart = node.orientation === 'horizontal' 
      ? containerRect.left 
      : containerRect.top;
    const totalSize = node.orientation === 'horizontal' 
      ? containerRect.width 

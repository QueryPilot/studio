...
import { IconClock, IconChevronDown, IconChevronRight } from '@tabler/icons-react';
...
export function SidebarRecent() {
  ...
  const handleConnect = async (profileId: string, name: string, database?: string) => {
    try {
        ...
        <div className="mt-1 space-y-0.5">
          {recentConnections.map((conn) => (
            <button
              ...
              type="button"
              onClick={() =>
                handleConnect(conn.profile.id, conn.profile.name, conn.profile.database)
              }
              className={cn(
            ...
            >
              ...
              />
              <span className="truncate text-[13px] flex-1 text-left">{conn.profile.name}</span>
            </button>
          ))}
        </div>
      )}

...
export function ConnectionForm() {
  ...
  const [dbType, setDbType] = useState<DatabaseType>(() => {
    if (connection) {
      ...
      return dbTypeMap[connection.profile.db_type as unknown as number] || 'postgresql';
    }
    return 'postgresql';
  });
  ...
  const [ssmTargetId, setSsmTargetId] = useState(existingAwsSsm?.target_id || '');
  ...
  const handleParseEnv = (text: string) => {
    try {
      ...
      setUriParsed(true);
      setTimeout(() => setUriParsed(false), 3000);
  ...
  const getTagColor = (tagName: string, isGroup: boolean = false) => {
    if (!isGroup) {
      ...
      if (groupTag) {
        ...
        return { bg: groupTag.color, text: colorObj?.textClass || 'text-white' };
      }
    }
  ...
  const buildConnectionProfile = (idOverride?: string): ConnectionProfile => {
    const resolvedId = idOverride ?? connection?.profile.id ?? `conn-${Date.now()}`;
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const connectionInfo = await invoke<{ id: string }>('connect', { profile });
                        ...
                        <CommandGroup heading="Environment" className="text-xs">
                          {ENVIRONMENT_TAGS.map((t) => (
                            <CommandItem
                              key={t.name}
                              value={t.name}
                              onSelect={() => handleTagToggle(t.name)}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <Checkbox
                          id="use-ssh-agent"
                          checked={useSSHAgent}
                          onCheckedChange={(checked) => setUseSSHAgent(!!checked)}
                        />
                        <Label htmlFor="use-ssh-agent" className="cursor-pointer text-xs">
                          Use SSH Agent
                        </Label>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => setUseSSHKey(!!checked)}
                        />
                        <Label
                          htmlFor="use-ssh-key"
                          className={cn(
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className="mt-1 h-8 text-xs"
                        value={ssmTargetId}
                        onChange={(e) => setSsmTargetId(e.target.value)}
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={() => setSsmAuthType('profile')}
                          className="h-3 w-3"
                        />
                  ...
                  {ssmAuthType === 'profile' ? (
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setSsmProfileName(e.target.value)}
                        placeholder="default"
                      />
                    </div>
                  ) : (
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setSsmOAuthClientId(e.target.value)}
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setSsmAssumeRoleArn(e.target.value)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        className="mt-1 h-8 text-xs"
                        value={ssmRemoteHost}
                        onChange={(e) => setSsmRemoteHost(e.target.value)}
                        placeholder={host}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        className="mt-1 h-8 text-xs"
                        value={ssmRemotePort}
                        onChange={(e) => setSsmRemotePort(e.target.value)}
                        placeholder={getDefaultPort(dbType)}
                      />
                    </div>
                  </div>
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-3 border-t border-border">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className={cn('h-7 px-2.5 text-xs', testSuccess && 'text-green-600')}
          >
            {isTesting ? (
              <>
